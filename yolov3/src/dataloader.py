...
class Dataset(torch.utils.data.Dataset): 
    def __init__( 
        self, csv_file, anchors,  
        image_size=416, grid_sizes=[13, 26, 52], 
        num_classes=20, transform=None, #image_dir, label_dir
    ): 
    ...
    def __getitem__(self, idx): 
        ...
        bboxes = [[float(self.label_info.iloc[idx, 1]), float(self.label_info.iloc[idx, 2]), float(self.label_info.iloc[idx, 3]), float(self.label_info.iloc[idx, 4]), 0]]
       
        # Getting the image path 
        img_path = self.label_info.iloc[idx, -1]
        ...
        for box in bboxes: 
            ...
            iou_anchors = helper.iou(torch.tensor(box[2:4]),  
                              self.anchors,  
                              is_pred=False) 
...
for i in range(y[0].shape[1]): 
    ...
    boxes += helper.convert_cells_to_bboxes( 
               y[i], is_predictions=False, s=y[i].shape[2], anchors=anchor 
             )[0] 
  
# Applying non-maximum suppression 
boxes = helper.nms(boxes, iou_threshold=1, threshold=0.7) 
